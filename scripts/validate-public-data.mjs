...
import { root, COMPONENTS, PUBLIC_KEYS, GRADES, SUBGRADES, gradeForScore, subgradeForScore, hasTierGate, assessEvidence, assertSafe, buildFaculty } from './build-public-data.mjs';
...
for(const record of faculty) {
  ...
  assert.equal(record.evidenceSubgrade,subgradeForScore(record.evidenceGrade,assessment.total));
...
assert.deepEqual(statistics.subgradeDistribution,Object.fromEntries(SUBGRADES.map(grade=>[grade,faculty.filter(r=>r.evidenceSubgrade===grade).length])));
